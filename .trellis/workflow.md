...
1. **Classify before code** — simple tasks can run directly; complex tasks need planning
...
```
Phase 1: Plan    → classify; simple tasks run directly, complex tasks get consent and planning artifacts
...
- A simple task must have a clear goal and acceptance criteria, be locally bounded, require no research, and avoid new dependencies, database changes, IPC/API contract changes, permission/security changes, concurrency/process changes, persistence protocol changes, and architecture changes. Typical examples: explanation, typo or copy fix, simple getter/setter, log addition, static style adjustment, or an obvious local fix.
- Simple tasks do not require task-creation consent or a Trellis task directory. Run the required start checks, implement directly, and validate the result.
- New features, unclear requirements, cross-module or cross-boundary changes, behavior/data-flow changes, high-risk changes, or anything whose classification is uncertain are complex tasks. Ask whether the user permits creating a Trellis task and entering planning. If the user says no, do not perform broad inline implementation.
- At the start of every task, including simple tasks, perform and report a read-only Git branch/upstream synchronization check. Do not automatically fetch, pull, push, merge, or rebase.
- For every code change, the finish gate must update both `CHANGELOG.md` and `docs/功能清单.md`. Ask for the changelog version when the user did not provide one; if the user still does not provide or explicitly declines, use `TEMP`.
...
No active task. First run the read-only Git branch/upstream synchronization check and report whether the branch is synchronized, ahead, behind, diverged, or has no upstream.
Simple task: when the goal is clear, locally bounded, research-free, and low-risk, do not ask for task-creation consent or create a task; implement and validate directly.
Complex or uncertain task: ask the user if you can create a Trellis task and enter planning. If the user says no, explain, clarify scope, or suggest a smaller split.
For every code change, ask for the changelog version if absent; if still unspecified, use `TEMP`, and before completion update both `CHANGELOG.md` and `docs/功能清单.md` in the relevant feature section.
[/workflow-state:no_task]

### Phase 1: Plan
- 1.0 Create task `[required · once]` (complex tasks only, after task-creation consent)
...
- Simple tasks do not require task creation or task-start approval. For complex tasks, task creation approval is not implementation approval; implementation waits for `task.py start` after artifact review.
...
Goal: classify the request, run the task-start checks, and for complex work get task-creation consent and produce the required planning artifacts before implementation. Simple work skips task creation and runs through the direct path.

#### 1.0 Create task `[required · once for complex tasks]`

Simple tasks skip this step. For complex tasks, create the task directory only after task-creation consent. The command sets status to `planning`, writes `task.json`, creates a default `prd.md`, and auto-targets the new task when session identity is available:
...
**Code-change record gate**: if the task changed code, confirm the `CHANGELOG.md` version before finalizing; ask when the user did not provide one, and use `TEMP` if the user still does not provide or explicitly declines. Update both `CHANGELOG.md` and the relevant section of `docs/功能清单.md` before drafting the commit plan. A task that only changes rules, workflow, or explanatory documentation is not a code change and does not require these two product records.
